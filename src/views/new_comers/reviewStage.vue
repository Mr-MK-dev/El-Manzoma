  ...
  methods: {
    ...
    async findItems(filters) {
      ...
      try {
        ...
        const Recommendations = await this.api("global/get_all", {
        ...
        const recommendationTN = Recommendations.data.map(ele => ele.tripleNo);
        const recommendationID = Recommendations.data.map(ele => ele.ID);
        let data = x.data;
        if (filters.isRecommendation) {
          data = x.data.filter(
            ele =>
              recommendationTN.includes(ele.TripleNo) ||
              recommendationID.includes(ele.ID)
          );
        }
        let printer = {
          data: [...data],
