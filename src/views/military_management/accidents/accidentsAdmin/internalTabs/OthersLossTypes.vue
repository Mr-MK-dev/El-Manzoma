<template>
  <div>
    <v-btn @click="actionAdd()" large outlined color="primary">
      إضافة نوع خسائر أخرى جديدة
    </v-btn>
    <table-bulider
      :headers="mainTable.headers"
      :items="mainTable.items"
      :title="'مواقف الخسائر الأخرى'"
    >
...
export default {
  ...
  methods: {
    ...
    async saveItem(edirableFromTableItem) {
      ...
      try {
        saveItem = await this.api(
          !this.otherLossType.ID_KEY ? `global/create_one` : `global/update_one`,
          {
            table: "OtherLossTypes",
            where: !this.otherLossType.ID_KEY
              ? this.otherLossType
              : {
                  ID_KEY: this.otherLossType.ID_KEY
                },
            update: this.otherLossType
          }
        );
    ...
    actionAdd() {
      this.$set(this, "otherLossType", { isEdit: false });
