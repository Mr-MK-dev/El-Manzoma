<template>
  <div>
    <v-btn @click="actionAdd()" large outlined color="primary">
      إضافة نوع حوادث أخرى جديدة
    </v-btn>
    <table-bulider
      :headers="mainTable.headers"
      :items="mainTable.items"
      :title="'انوااع الحوادث'"
    >
...
export default {
  ...
  methods: {
    ...
    async saveItem(edirableFromTableItem) {
      ...
      try {
        saveItem = await this.api(
          !this.accidentType.ID_KEY ? `global/create_one` : `global/update_one`,
          {
            table: "accidentTypes",
            where: !this.accidentType.ID_KEY
              ? this.accidentType
              : {
                  ID_KEY: this.accidentType.ID_KEY
                },
            update: this.accidentType
          }
        );
    ...
    actionAdd() {
      this.$set(this, "accidentType", { isEdit: false });
