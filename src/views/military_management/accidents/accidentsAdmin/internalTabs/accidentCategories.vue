<template>
  <div>
    <v-btn @click="actionAdd()" large outlined color="primary">
      إضافة تصنيف حوادث جديد
    </v-btn>
    <table-bulider
      :headers="mainTable.headers"
      :items="mainTable.items"
      :title="'تصنيفات الحوادث'"
    >
...
<script>
export default {
  name: "accidentCategoriesManagement",
  mounted() {
    this.findAllTypes();
  },
  data: () => ({
  ...
  methods: {
    ...
    async saveItem(edirableFromTableItem) {
      ...
      try {
        saveItem = await this.api(
          !this.accidentCategory.ID_KEY
            ? `global/create_one`
            : `global/update_one`,
          {
            table: "accidentCategories",
            where: !this.accidentCategory.ID_KEY
              ? this.accidentCategory
              : {
                  ID_KEY: this.accidentCategory.ID_KEY
                },
            update: this.accidentCategory
          }
        );
    ...
    actionAdd() {
      this.$set(this, "accidentCategory", { isEdit: false });
    ...
    actionEdit(item) {
      this.$set(this.createdObject, "model", true);
      this.$set(this, "accidentCategory", {
        ...item,
        isEdit: true
      });
    },
