<template>
  <div>
    ...
    <table-bulider
      ...
      :wordFile="'/officers_management/battalion_speech'"
      :isExcel="true"
      :title="'الخطاب'"
  ...
  data: () => ({
    ...
    mainTable: {
      headers: [
        ...
        {
          text: "جهة المأمورية والغرض منها",
          value: "Notes",
          searchValue: "Notes",
          multiple: false,
          sortable: true,
  ...
  methods: {
    async findAllVacations(filters) {
      ...
      try {
        ...
        let items = [...courses.data, ...vacations.data, ...missions.data];
        if (filters.Date.length > 0) {
          items = items.filter(ele => ele.DateFrom == filters.Date[0]);
        }
        items = items.map(ele => ({
          ...ele,
          id: id++,
          CurrentDate: filters.Date[0],
          Commitment: ele.CourseID
            ? ele.Course.Name
            : ele.VacationID
            ? ele.Vacation.Name
            : ele.Mission.Name
