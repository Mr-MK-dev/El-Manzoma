<template>
  <div>
    <search-bulider
      ...
      @on-search="findAllMissions"
      @on-add="actionAdd()"
      :search="search"
      :clearOption="true"
      title="بحث متقدم "
      :addButton="true"
      addBtnTitle="إضافة مأمورية"
    />
    <table-bulider
...
export default {
  ...
  mounted() {
    ...
    this.findAllMissions(this.search);
  },
  data: () => ({
    del: {
      loading: false,
  ...
  methods: {
    findOne(ID) {
      if (ID && ID.length > 1) {
        this.api("global/get_one", {
          ...
          .then(x => {
            console.log(x);
            this.$set(this.mission, "ID", x.data.ID);
          })
          .catch(error => {})
          .finally(() => {});
      }
    ...
    async saveItem(edirableFromTableItem) {
      ...
      let saveItem;
      if (await this.checkIfOfficerNotAvailable(this.mission)) {
        return this.showError(`هذا الفرد له التزام اخر`);
      }

      try {
        saveItem = await this.api(
          !this.mission.isEdit ? `global/create_one` : `global/update_one`,
          {
            table: "OfficerMissions",
            where: !this.mission.isEdit
              ? this.mission
              : {
                  ID_KEY: this.mission.ID_KEY
                },
            update: this.mission
          }
        );
    ...
    actionAdd() {
      this.$set(this, "mission", { isEdit: false });
