<template>
  <div>
    ...
    <table-bulider
      ...
      :title="'الأنماط'"
    >
      <template v-if="hasEditAccess()" v-slot:item.edit="{ item }">
...
export default {
  ...
  methods: {
    ...
    async saveItem(edirableFromTableItem) {
      ...
      try {
        saveItem = await this.api(
          `global/${this.pattern.isEdit ? "update_one" : "create_one"}`,
          {
            table: "Pattern",
            where: !this.pattern.isEdit
              ? this.pattern
              : {
                  ID_KEY: this.pattern.ID_KEY
                },
            update: this.pattern
          }
        );
    ...
    actionAdd() {
      this.$set(this, "pattern", { isEdit: false });
