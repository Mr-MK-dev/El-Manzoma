  ...
  methods: {
    ...
    async checkPassword() {
      ...
      if (password == truePassword) {
        ...
        deviceData = await this.api(`global/get_ip`);
        let create = await this.api(`global/create_one_get`, {
          table: "SessionTracking",
          where: {
            userId: this.user.userId,
            IP: deviceData?.data[0]?.address || null,
            MAC: deviceData?.data[0]?.mac || null,
            SessionStartDate: date
          }
        });
        this.$set(this.$store.state, "currentUser", {
          ...this.user,
          SessionID: create?.data?.ID_KEY || null
        });
        this.getSockets();
        // if (saveLogin) {
