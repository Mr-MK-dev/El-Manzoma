...
export default {
  ...
  methods: {
    ...
    async saveItem(edirableFromTableItem) {
      ...
      try {
        saveItem = await this.api(
          !this.appealType.ID_KEY ? `global/create_one` : `global/update_one`,
          {
            table: "AppealType",
            where: !this.appealType.ID_KEY
              ? this.appealType
              : {
                  ID_KEY: this.appealType.ID_KEY
                },
            update: this.appealType
          }
        );
    ...
    actionAdd() {
      this.$set(this, "appealType", { isEdit: false });
