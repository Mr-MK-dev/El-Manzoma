                  ...
                  <v-card-text>
                    <v-avatar size="220">
                      <div>
                        <div class="mb-2 mt-4">
                          <span class="text-h5 primary--text">
                            {{ this.internalFollowUnFinished }}
                          </span>
                        </div>
                        <div>
                          <v-card-title
                            class="d-block break-word"
                            v-text="'هناك  متابعة داخلية لم تنتهي'"
                          ></v-card-title>
                        </div>
                      </div>
                    </v-avatar>
...
export default {
  ...
  mounted() {
    ...
    this.findInternalFollowUnFinished();
    // this.$nextTick(() => {
  ...
  methods: {
    findUnReadMessages() {
      this.api("global/get_all", {
        ...
        where: { User_2: this.$store.state.currentUser.userId }
      })
        .then(x => {
          let data = x.data;
          data = data.filter(ele => !ele.seen).length;
          console.log("data", data);
          this.$set(this, "unRead", data);
        })
        .catch(error => {
    ...
    findInternalFollowUnFinished() {
      let where = {
          GroupId: [this.$store.state.currentUser.GroupIds]
        },
        likes = ["GroupId"],
        multi = [];
      ...
      this.api("global/get_all", {
        ...
        .then(x => {
          let data = x.data;
          data = data.filter(ele => !ele.isFinished).length;
          this.$set(this, "internalFollowUnFinished", data);
        })
        .catch(error => {
          console.log(error);
          this.showError("حدث خطأ أثناء احضار البيانات من قاعدة البيانات");
    ...
    findFollowUnFinished() {
      ...
      this.api("global/get_all", {
        ...
        .then(x => {
          let data = x.data;
          data = data.filter(ele => !ele.isApproved).length;
          this.$set(this, "followUnFinished", data);
        })
        .catch(error => {
          console.log(error);
          this.showError("حدث خطأ أثناء احضار البيانات من قاعدة البيانات");
