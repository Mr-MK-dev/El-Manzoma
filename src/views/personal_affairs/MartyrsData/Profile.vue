  ...
  <div tabindex="0" class="no-focus profile">
    <template v-for="(group, i) in groups">
      <v-card class="mb-8" :key="i">
        ...
        ></print-conscripte-profile>
        <v-card-title class="primary white--text" v-text="group.title">
        </v-card-title>
...
export default {
  ...
  methods: {
    ...
    async updateImageApi(image) {
      ...
      this.showSuccess("تم إضافة الصورة بنجاح");
    },
    findItems(filters) {
      ...
      this.api("global/get_one", {
        ...
        .then(async x => {
          let martyr = await this.fetchDataBaseOnType(
            this.id,
            x.data.Type == "مجند"
              ? "Soldier"
              : x.data.Type == "ضابط"
          ...
          let data = {
            ...
            ...parentObject,
            ...martyr.Martyr,
            UnitName: martyr.Martyr.Unit.Unit,
            Direction: martyr.Martyr.Unit.Directionforunit,
            WeaponName: martyr.Martyr.Weapon.Weapon,
