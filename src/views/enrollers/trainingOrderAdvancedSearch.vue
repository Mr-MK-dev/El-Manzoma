<template>
  <div>
    <search-bulider
      ...
      :clearOption="true"
      :search="search"
      title="بحث متقدم فأمر مهمة تدريبية"
      addBtnTitle="إضافة مهمة تدريبية"
    />
    <table-bulider
      :isExcel="true"
    ...
    >
      ...
      <template v-slot:item.isTrainingOrder="{ item }">
        <v-chip
          @click="actionEndTraining(item)"
          :color="item.isTrainingOrder == 1 ? 'success' : 'error'"
        >
...
export default {
  ...
  data: () => ({
    ...
    mainTable: {
      ...
      headers: [
        ...
        {
          text: "العمل القائم به في مكان المهمة التدريبية",
          value: "FollowerDuty",
        ...
        {
          text: "تاريخ المهمة التدريبية",
          value: "TrainingOrderDate",
          searchValue: "TrainingOrderDate",
          type: "date",
          sortable: true,
          inTable: true,
          inModel: true,
    ...
    selects: {
      ...
      isTrainingOrder: {
        ...
        value: "value",
        data: [
          { text: "مستمر مهمته التدريبية", value: 1 },
          { text: "غير مستمر مهتمه التدريبية", value: 0 }
        ]
      },
      isIllegal: {
        text: "text",
  ...
  methods: {
    actionEndTraining(item) {
      this.$confirm(`هل انت متاكد من تغير الحالة`, {
        title: ``
      })
        .then(async res => {
          if (res) {
            if (item.isTrainingOrder) {
              await this.api(`global/update_one`, {
                table: `${
                  item.Type == "مجند" ? "TrainingSoldier" : "TrainingRateb"
                }`,
                where: {
                  ID_KEY: item.ID_KEY
                },
    ...
    async checkIfEnrolledAfterEnrollDate() {
      ...
      try {
        let soldier = await this.api(`global/get_all`, {
          table: "TrainingSoldier",
          where: { $or: { isTrainingOrder: true } }
        });
        ...
        if (check.length > 0)
          return this.showError(
            "هناك أمر مهمة تدريبية إنتهى تاريخ مهمتهم ولم يتم حذف المهمه"
          );
      } catch (e) {
        this.handleError(e);
      }
    ...
    async saveEffect() {
      ...
      try {
        saveItem = await this.api(`global/update_one`, {
          table: `${
            this.effect.Type == "مجند" ? "TrainingSoldier" : "TrainingRateb"
          }`,
    ...
    async saveItem(edirableFromTableItem) {
      ...
      if (
        ...
        this.showError("هذا الرقم العسكري تم اضافته بمهمه بالفعل");
        return;
      }
      // if (!this.enrollers.isEdit) {
      //   this.enrollers.isTrainingOrder = 1;
      // }
    ...
    ActionEndEnrollment(item) {
      ...
      if (this.hasEditAccess()) {
        this.$confirm(`هل انت متاكد من إنهاء المهمة التدريبية `, {
          title: ``
        }).then(async res => {
          if (res) {
            this.$set(this, "effect", { ...item, isTrainingOrder: false });
    ...
    async isDuplicate(ID, Type) {
      ...
      data = await this.api("global/get_one", {
        table: `${Type == "مجند" ? "TrainingSoldier" : "TrainingRateb"}`,
        where: { ID, isTrainingOrder: true }
      });
    ...
    async searchFollowers(filters) {
      ...
      if (!filters?.Type?.length) {
        this.showError("برجاء اختيار نوع الفرد ");
        return;
      }
      ...
      this.api("sections/tasgeel/search/trainers", {
