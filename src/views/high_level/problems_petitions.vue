  ...
  methods: {
    ...
    findOne(id) {
      if (id.length > 10) {
        this.api("global/get_one", {
          ...
          .then(x => {
            this.$set(this.problem, "Name", x.data.Name);
          })
          .catch(error => {})
          .finally(() => {});
      }
    ...
    findItems() {
      ...
      this.api("global/get_all", {
        table: "shakawi",
        include: [{ model: "Rateb" }],
        where: this.mapToQuery(where, likes, multi, data)
      })
        .then(x => {
          console.log(x);
          let data = x.data,
