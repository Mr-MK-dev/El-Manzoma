  ...
  watch: {
    ...
    async "search.collections"(newValue) {
      ...
      if (this.search.Type == displayTypes.collections) {
        ...
        this.$set(this.search, "unitIds", data.data.map(ele => ele.UnitID)
        );
      ...
      try {
        ...
        for (const unit of unites.data) {
          ...
          items.push({
            ...
            Siasa: lodash.sumBy(mortabs.data, ele => ele.Siasa),
            mos2dA: rateb.data.filter(ele =>
              ele.RatebLevel.includes("مساعد اول")
            ).length,
            mos2d: rateb.data.filter(
              ele =>
                ele.RatebLevel.includes("مساعد") &&
                !ele.RatebLevel.includes("مساعد اول")
            ).length,
            raqebA: rateb.data.filter(ele =>
              ele.RatebLevel.includes("رقيب اول")
            ).length,
            raqeb: rateb.data.filter(
              ele =>
                ele.RatebLevel.includes("رقيب") &&
                !ele.RatebLevel.includes("رقيب اول")
            ).length,
            aref: rateb.data.filter(ele => ele.RatebLevel.includes("عريف"))
              .length
          });
        }
        items.push({
