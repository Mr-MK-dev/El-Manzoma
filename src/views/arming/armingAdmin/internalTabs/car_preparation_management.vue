<template>
  <div>
    ...
    <table-bulider
      ...
      :title="'تجهيزات المركبات'"
    >
      <template v-slot:item.edit="{ item }">
        <v-chip color="transparent" @click="deleteCarPreparation(item)">
...
<script>
export default {
  name: "car_preparation_management",
  mounted() {
    this.findAllCarPreparations();
  },
  data: () => ({
    del: {
      loading: false,
  ...
  methods: {
    ...
    async saveItem(edirableFromTableItem) {
      ...
      try {
        saveItem = await this.api(
          !this.carType.ID_KEY ? `global/create_one` : `global/update_one`,
          {
            table: "CarPreparation",
            where: !this.carType.ID_KEY
              ? this.carType
              : {
                  ID_KEY: this.carType.ID_KEY
                },
            update: this.carType
          }
        );
    ...
    actionAdd() {
      this.$set(this, "carType", { isEdit: false });
    ...
    actionEdit(item) {
      this.$set(this.createdObject, "model", true);
      this.$set(this, "carType", {
        ...item,
        isEdit: true
      });
    },

    async deleteCarPreparation(item) {
