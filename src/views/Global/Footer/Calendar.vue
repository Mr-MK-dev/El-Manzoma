...
export default {
  ...
  watch: {
    ...
    "currentEvent.item.typeId"(v) {
      let fields = this.eventFields.filter(f =>
          ...
          ["sectionId", "degreeId"].includes(f.value)
        ),
        disabled = false;
      if ([3, 4].includes(v)) {
        // لو أجازة أو راحة
  ...
  methods: {
    ...
    saveEvent() {
      ...
      this.api(`global/${isUpdate ? "update_one" : "create_one"}`, {
        table: "Calendars",
        where: isUpdate
          ? {
              id: event.id
    ...
    getEvents() {
      ...
      let { isAdmin, GroupIds } = this.$store.state.currentUser,
        where = {};
      if (!isAdmin) {
        where = {
          $or: [
            {
              sectionId: {
                $in: JSON.parse(GroupIds)
              },
              AddedBy: {
                $eq: this.$store.state.currentUser.userId
              }
            }
          ]
        };
