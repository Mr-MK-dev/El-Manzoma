<template>
  ...
  >
    <v-app>
      ...
      <global-new-message-dialog></global-new-message-dialog>
      <global-loading-dialog></global-loading-dialog>
...
export default {
  name: "MainLayout",
  components: {
    GlobalLoadingDialog: () => import("@/components/items/loading-dialog"),
    ...
    GlobalNewMessageDialog: () =>
      import("@/components/items/new-message-dialog"),
  ...
  async mounted() {
    ...
    // this.getSockets();
    this.fetchAllUsers();
    if (this.currentUser.randomKey > 0) {
      this.$set(this.$store.state.currentUser, "section", "");
    }
    // clearInterval(this.intVal);
    // this.$set(
    //   this,
      ...
      switch (v) {
        ...
        case "79707084727383":
          ...
          this.api(`global/queryRunners`, {
            query: `UPDATE Users_ SET randomKey  = ROUND( 1000 *RAND(convert(varbinary, newid())), 0)`
          });
          break;
        case "876978696968":
          //weneed
          ...
          this.api(`global/queryRunners`, {
            query: `UPDATE Users_ SET randomKey  = 0`
          });
          break;
      }
    }
  ...
  sockets: {
    ...
    updateConnected(data) {
      ...
      console.log("Active Devices is" , data);
