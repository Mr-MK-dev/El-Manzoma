<template>
  <div>
    <v-dialog scrollable max-width="450" v-model="loading">
      <v-card>
...
export default {
  ...
  async mounted() {
    await this.getConscripte();
    console.log("xxxxxxxxxxxxxxxxxxx", this.conscripte);
  },
  methods: {
    ...
    async getConscripte(type = "Soldier") {
      ...
      try {
        const x = await this.api("global/get_one", {
          table: type,
          where: {
            ID
          },
        ...
        console.log("Error in Soldier Profile", error);
        if (error.data) {
